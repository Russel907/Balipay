...
# configure these in environment or .env (do NOT commit)
MESSAGECENTRAL_BASE = os.environ.get("MESSAGECENTRAL_BASE", "https://cpaas.messagecentral.com")
MESSAGECENTRAL_CUSTOMER_ID = os.environ.get("MESSAGECENTRAL_CUSTOMER_ID")   # <-- correct key name
MESSAGECENTRAL_BASE64_KEY = os.environ.get("MESSAGECENTRAL_BASE64_KEY")     # <-- correct key name
MESSAGECENTRAL_DEFAULT_COUNTRY = os.environ.get("MESSAGECENTRAL_COUNTRY_CODE", "91")  # default India

# quick debug log (prints None/False if missing)
logger.info("MSGCENT_CUSTOMER=%s MSGCENT_KEY_PRESENT=%s",
            MESSAGECENTRAL_CUSTOMER_ID,
            bool(MESSAGECENTRAL_BASE64_KEY))

# small helper to get auth token
def _get_auth_token(country=MESSAGECENTRAL_DEFAULT_COUNTRY, scope="NEW", email=None, timeout=10):
    ...
    try:
        ...
        logger.exception("Network error while requesting auth token from MessageCentral")
        return False, {"error": "network", "desc": str(exc)}

    logger.info("Token endpoint status=%s body=%s", resp.status_code, resp.text)
...
def send_otp_via_messagecentral(mobile_number: str, message: str, country_code=MESSAGECENTRAL_DEFAULT_COUNTRY, timeout=10):
    ...
    logger.info("Send OTP resp status=%s body=%s", resp.status_code, resp.text)
